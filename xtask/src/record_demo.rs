...
/// Workspace-relative directory of the shared automation library package.
const AUTOMATION_DIR: &str = "automation";
/// Workspace-relative directory of the demo package.
...
pub trait DemoSystem {
    ...
    ///
    /// # Returns
    /// Path to the built `cssh-rs` executable.
    ///
    /// # Errors
...
///
/// Builds the binary, installs the automation and demo packages, runs the
/// demo recorder, and verifies the GIF was written. The GIF lands under
/// `target/` so it inherits Cargo's `.gitignore`; publishing it is a separate
/// step.
///
/// # Arguments
...
pub fn record_demo<S: DemoSystem>(system: &S) -> Result<()> {
    if !system.is_windows() {
        bail!(
            "`record-demo` runs on Windows only: it synthesises keystrokes into and captures real console windows"
        );
    }
    ...
    for package in [AUTOMATION_DIR, DEMO_DIR] {
