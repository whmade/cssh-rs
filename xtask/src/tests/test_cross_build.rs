...
use crate::cross_build::{run_cross_build, CrossBuildSystem, Target};
...
mock! {
    ...
    impl CrossBuildSystem for CrossBuildSystemMock {
        ...
        fn install_cargo_subcommand(&self, crate_name: &str) -> anyhow::Result<()>;
        fn run_cargo_build(&self, triple: &str) -> anyhow::Result<()>;
        fn run_cargo_xwin_build(&self, triple: &str) -> anyhow::Result<()>;
...
fn test_cross_build_windows_host_uses_native_cargo_build() {
    ...
    // Act
    let result = run_cross_build(&mock, Target::X86_64PcWindowsMsvc);
...
fn test_cross_build_installs_missing_rust_target() {
    ...
    mock.expect_install_target()
        ...
        .returning(|_| Ok(()));
    mock.expect_is_executable_in_path().never();
    mock.expect_run_cargo_build().returning(|_| Ok(()));
...
#[test]
fn test_cross_build_installs_missing_cargo_xwin() {
    // Arrange
    let mut mock = base_mock();
    mock.expect_host_os().returning(|| "linux");
    mock.expect_list_installed_targets()
    ...
    mock.expect_install_cargo_subcommand()
        .withf(|c| c == "cargo-xwin")
        .times(1)
        .returning(|_| Ok(()));
    mock.expect_run_cargo_xwin_build().returning(|_| Ok(()));
...
#[test]
fn test_cross_build_unsupported_host_errors_before_any_work() {
    // Arrange
    let mut mock = base_mock();
    mock.expect_host_os().returning(|| "freebsd");
    // No prerequisite check or build call should fire when the
    // (host, target) pair is unsupported.
...
fn test_cross_build_propagates_build_failure() {
    ...
    mock.expect_list_cargo_subcommands()
        .returning(|| Ok("Installed Commands:\n    xwin\n".to_owned()));
    mock.expect_run_cargo_xwin_build()
        .returning(|_| anyhow::bail!("link.exe equivalent crashed"));
