...
fn test_missing_token_file_is_noop() {
    ...
    testing_logger::validate(|logs| {
        ...
        assert!(
            logs[0].body.contains("no ") && logs[0].body.contains("gh-token"),
            "log should mention the missing token file: {}",
            logs[0].body
        );
        assert!(
            logs[0].body.contains("CONTRIBUTING.md"),
...
fn test_classic_token_is_accepted_with_warning() {
    ...
    testing_logger::validate(|logs| {
        let warn = logs
            .iter()
            .find(|log| log.level == Level::Warn)
            .expect("classic token must emit a Warn log");
        assert!(
            warn.body.contains("classic"),
            "warning must name the token kind: {}",
            warn.body
        );
        assert!(
            warn.body.contains("fine-grained"),
            "warning must recommend fine-grained PATs: {}",
            warn.body
        );
        assert!(
            warn.body.contains("CONTRIBUTING.md"),
            "warning must point at CONTRIBUTING.md: {}",
            warn.body
        );
    });
}
...
fn test_oauth_token_is_accepted_with_warning() {
    ...
    testing_logger::validate(|logs| {
        let warn = logs
            .iter()
            .find(|log| log.level == Level::Warn)
            .expect("OAuth token must emit a Warn log");
        assert!(
            warn.body.contains("OAuth"),
            "warning must name the token kind: {}",
            warn.body
        );
        assert!(
