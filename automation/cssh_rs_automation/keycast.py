...
class Keycast:
    ...
    def record(self, label: str, kind: str) -> None:
        ...
        characters read as continuous text; a ``"key"`` event starts a new token.
        ...
        with self._lock:
            now = time.monotonic()
            if kind == "text" and self._events:
                last_label, last_kind, last_stamped = self._events[-1]
                if last_kind == "text" and now - last_stamped <= self._fade_seconds:
                    self._events[-1] = (last_label + label, "text", now)
                    return
...
def _keycast_text(events: list[tuple[str, str]]) -> str:
    """Return the overlay line: text tokens verbatim, key tokens upper-cased, space-joined."""
