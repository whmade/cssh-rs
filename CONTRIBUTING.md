...
To update it, trigger the **Regenerate demo GIF** workflow (Actions tab -> "Run workflow") with a `target` branch. The workflow records the clip from that branch, validates the GIF, and opens a review PR against `demo-assets` so you can inspect the rendered old/new GIF diff before it goes live. Merging that PR publishes the clip.

Each branch keeps its own clip on the shared `demo-assets` branch, keyed by `target`:
