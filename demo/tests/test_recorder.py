...
def test_start_demo_seeds_hosts_wires_overlay_and_launches(
    ...
    mocks["sshd"].start_sshd.assert_called_once_with(
        HOSTS, interactive=True, rc_lines=SHELL_RC_LINES
    )
    ...
    # Titles show root; the real login is forced back with -o User=.
    assert generate_kwargs["extra_ssh_args"] == ["-o", f"User={getpass.getuser()}"]
    ...
    # Only the README host is seeded (with a trailing newline), and the
    # recording starts before the launch.
...
def test_broadcast_focuses_daemon_then_types_each_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    typed = [c.args[0] for c in mocks["keystrokes"].type_text.call_args_list]
    assert typed == ["h", "i"]
...
def test_add_host_types_the_alias_then_waits_for_its_session(
    ...
    expected = (
        ...
        + [call.press_key("backspace", notify=False)]
    )
    assert mocks["keystrokes"].method_calls == expected
...
def test_tear_down_demo_is_best_effort(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr("cssh_rs_demo.recorder.subprocess.run", MagicMock(name="run"))
